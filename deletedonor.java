...
public class deletedonor extends javax.swing.JFrame {
    ...
    public void actionPerformed(ActionEvent e) {
        String donorId = jTextField1.getText();
        int confirm = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this donor?", "Confirm", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/bloodmanagementsystem", "root", "Priti@3204");
                PreparedStatement pst = con.prepareStatement("DELETE FROM donors WHERE donor_id = ?");
                pst.setString(1, donorId);
                int rows = pst.executeUpdate();
                if (rows > 0) {
                    JOptionPane.showMessageDialog(null, "Donor deleted successfully.");
                    
                    // Reset all fields directly
                    jTextField1.setText("");
                    jTextField2.setText("");
                    jTextField3.setText("");
                    jTextField4.setText("");
